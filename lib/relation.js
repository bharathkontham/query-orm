...
class BelongsTo {
  constructor(self, property) {
    self.relations[property] = self.relations[property] || {};
    self.relations[property]['findOne'] = async (relationId, filter) => {
      let query = {
        where: {}
      };
      query.where[self.relationDefinitions[property].keyTo] = relationId;
    ...
    self.relations[property]['setOrCreate'] = async (relationId, data) => {
      let where = {};
      where[self.relationDefinitions[property].keyTo] = data[self.relationDefinitions[property].keyFrom]
      const relatedData = await scopeMethods['findOne'](query, self.relationDefinitions[property].modelTo)
      if (relatedData) {
        data[self.relationDefinitions[property].keyTo] = relationId;
        return await scopeMethods['updateByQuery'](where, data, self.relationDefinitions[property].modelTo)
      } else {
        data[self.relationDefinitions[property].keyTo] = relationId;
        return await scopeMethods['create'](data, self.relationDefinitions[property].modelTo)
      }
